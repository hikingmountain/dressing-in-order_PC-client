...
class DFVisualDataset(DFPairDataset):
    ...
    def __init__(self, dataroot, dim=(256,256), texture_dir="",isTrain=False, n_human_part=8):
        ...
        self.garment_image_dir = dataroot +  "/test"
        self.model_image_dir = dataroot + "/model"
    ...
    def get_inputs_by_key(self, key, imgType):
        #keys = self.attr_keys[subset]
        #keys = keys[:min(len(keys), 4)]
        if (imgType == 'model'):
            curr_from = self._load_img(os.path.join(self.model_image_dir, key))
        else:
            curr_from = self._load_img(os.path.join(self.garment_image_dir, key))

        to_kpt = self._load_kpt(key)
        curr_parse = self._load_mask(os.path.join(self.mask_dir, key[:-4]))
